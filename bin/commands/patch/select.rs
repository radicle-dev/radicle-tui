...
impl TryFrom<&Context> for State {
    ...
    fn try_from(context: &Context) -> Result<Self, Self::Error> {
        ...
        let view = ItemView::new(items).filter(filter);
        // view.filter(filter);
    ...
    fn handle_action(&mut self, action: Action) -> Option<Exit<Selection>> {
        match action {
            ...
            Action::UpdateSearch { value } => {
                ...
                // let filter = PatchItemFilter::from_str(&self.browser.search.read()).unwrap_or_default();
                // let items = self.browser.patches.filter(filter);
                // let mut patches = self.browser.patches.clone().iter();
                // patches.filter(
