...
impl<'a> From<&State> for BrowsePageProps<'a> {
    fn from(state: &State) -> Self {
        ...
        let notifications = state.browser.items.all();
...
impl<'a: 'static> Widget for BrowsePage<'a> {
    ...
    fn new(state: &State, action_tx: UnboundedSender<Action>) -> Self {
        ...
        Self {
            ...
            notifications: Container::new(state, action_tx.clone())
                ...
                .content(Box::<
                    Table<State, Action, NotificationItem, NotificationItemFilter>,
                >::new(
    ...
    fn handle_event(&mut self, key: Key) {
        let notifications = self.props.notifications.all().collect::<Vec<_>>();
...
fn browse_footer<'a>(props: &BrowsePageProps<'a>, selected: Option<usize>) -> Vec<Column<'a>> {
    ...
    let progress = selected
        .map(|selected| {
            TableUtils::progress(
                selected,
                props.notifications.all().collect::<Vec<_>>().len(),
                props.page_size,
            )
        })
        .unwrap_or_default();
