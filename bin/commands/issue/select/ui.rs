...
impl<'a> From<&State> for BrowsePageProps<'a> {
    fn from(state: &State) -> Self {
        ...
        let issues = state.browser.items.all();
    ...
    fn new(state: &State, action_tx: UnboundedSender<Action>) -> Self {
        ...
        Self {
            ...
            issues: Container::new(state, action_tx.clone())
                ...
                .content(
                    Box::<Table<State, Action, IssueItem, IssueItemFilter>>::new(
        ...
        if self.props.show_search {
            ...
            match key {
                ...
                Key::Char('\n') => {
                    ...
                    let issues = self.props.issues.all().collect::<Vec<_>>();
...
fn browse_footer<'a>(props: &BrowsePageProps<'a>, selected: Option<usize>) -> Vec<Column<'a>> {
    let issues = &props.issues.all().collect::<Vec<_>>();
